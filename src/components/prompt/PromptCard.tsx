...
export const PromptCard = ({
  ...
  return (
    <div className={`relative h-full ${aspectRatio}`}>
      <Link href={`/prompt/${id}`} className="block h-full">
        <Card className="h-full w-full overflow-hidden rounded-2xl flex flex-col border-border/40 bg-card/60 backdrop-blur-xl relative">
          
          <div className="relative w-full h-full bg-muted/30 overflow-hidden z-20">
            {isCode ? (
              <div className="absolute inset-0 bg-[#0d1117] p-5 font-mono text-[10px] leading-relaxed text-[#c9d1d9] flex flex-col shadow-inner select-none">
                ...
                <div className="flex-1 overflow-hidden opacity-90 whitespace-pre">
              ...
              <>
                <img src={previewImage} alt={title} className="absolute inset-0 w-full h-full object-cover z-10" />
                {isVideo && (
