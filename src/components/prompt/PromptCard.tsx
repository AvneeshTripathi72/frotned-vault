...
export const PromptCard = ({
  id, title, tagline, short_description, price, rating, platform, author, previewImage, promptPreview,
}: PromptCardProps) => {
        ...
        <Card className="glass-card h-full overflow-hidden rounded-2xl flex flex-col border-border/40 bg-card/60 backdrop-blur-xl transition-all duration-300 relative">
          ...
          <div className="relative aspect-square bg-muted/30 overflow-hidden z-20">
            <AnimatePresence mode="wait">
              {isHovered && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-30 flex flex-col justify-center items-center text-center p-6">
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-3">
                    ...
                    <p className="text-[12px] font-bold text-white italic line-clamp-4 max-w-[180px]">"{promptPreview}"</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {previewImage ? (
              <img src={previewImage} alt={title} className="absolute inset-0 w-full h-full object-contain z-10 p-2 transition-transform duration-500 ease-out group-hover:scale-105" />
            ) : (
              <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-muted/10 z-10"><Sparkles className="w-6 h-6 text-muted-foreground/10" /></div>
            )}
          ...
          <div className="p-4 flex flex-col gap-3 flex-grow relative z-20">
            <div className="space-y-1">
              <div className="flex justify-between items-start gap-2">
                <h3 className="font-bold text-[15px] leading-tight tracking-tight text-foreground line-clamp-1 group-hover:text-primary">{title}</h3>
                <div className="flex items-center gap-0.5 text-yellow-500 shrink-0"><Star className="w-3 h-3 fill-current" /><span className="text-[11px] font-bold">{rating}</span></div>
              </div>
              <p className="text-[12px] text-muted-foreground line-clamp-1 font-medium opacity-80">{short_description || tagline}</p>
            </div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-primary/20 overflow-hidden border border-border/50 flex items-center justify-center">
                  {author.avatar && !imgError ? (
                    <img src={author.avatar} alt="" className="w-full h-full object-cover" onError={() => setImgError(true)} />
                  ) : (
                    <span className="text-[10px] font-black text-primary uppercase">{(author.username?.[0] || 'U').toUpperCase()}</span>
                  )}
                </div>
